...
class EnemyNode: SKSpriteNode {
    ...
    init(type: EnemyType, startPosition: CGPoint, xOffset: CGFloat, moveStraight: Bool) {
        ...
        // bounce off the player or their weapon
    ...
    func configureMovement(_ moveStraight: Bool) {
        ...
        // remove the thing as soon as its finished 
    ...
    func fire() {
        ...
        weapon.physicsBody?.mass = 0.001 // all weapon types have the same mass, and thus the same speed
        
        // now, must push the weapon in the correct direction, which requires a little mass
        let speed: CGFloat = 1
        let adjustedRotation = zRotation + (CGFloat.pi / 2) // must flip this because +x points to the left
        
        let dx = speed * cos(adjustedRotation)
        let dy = speed * sin(adjustedRotation)
        
        weapon.physicsBody?.applyImpulse(CGVector(dx: dx, dy: dy)) // bullet flies off in the direction that the enemy was facing when it fired
